'use client';
import React, { useState, useEffect } from 'react';
import axios from 'axios';
import Footer from '@/components/footer/footer';
import Navbar from '@/components/navbar/navbar';

interface SoftwareSubscriptionPlan {
  id: number;
  name: string;
  description: string;
  price: number;
  features: string[];
}

const SoftwareSubscriptionSection: React.FC = () => {
  const [subscriptionPlans, setSubscriptionPlans] = useState<SoftwareSubscriptionPlan[]>([]);

  useEffect(() => {
    const fetchSubscriptionPlans = async () => {
      try {
        const response = await axios.get('/api/software-subscription-plans');
        setSubscriptionPlans(response.data);
      } catch (error) {
        console.error('Error fetching software subscription plans:', error);
      }
    };

    fetchSubscriptionPlans();
  }, []);

  return (
    <div className="min-h-screen bg-black text-white">
      <Navbar />
      <div className="container mx-auto px-4 py-8">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-primary mb-4">Software Subscriptions</h1>
          <p className="text-gray-400 text-lg">Access our premium software with a subscription plan.</p>
        </div>

        {subscriptionPlans.length === 0 ? (
          <div className="text-center py-16">
            <p className="text-gray-400 text-xl">No subscription plans available at the moment.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
            {subscriptionPlans.map((plan) => (
              <div
                key={plan.id}
                className="rounded-2xl bg-gray-900 hover:bg-gray-800 border border-gray-700 hover:border-primary transition-all duration-300 group shadow-xl overflow-hidden p-6 flex flex-col justify-between max-w-[300px]"
              >
                <div>
                  <h3 className="text-xl font-bold mb-2">{plan.name}</h3>
                  <p className="text-gray-400 text-sm group-hover:text-white transition-colors duration-300 mb-2">
                    {plan.description}
                  </p>
                  <p className="text-gray-400 text-sm group-hover:text-white transition-colors duration-300 mb-4">
                    Price: ${plan.price} per month
                  </p>
                  <div>
                    <h4 className="text-lg font-bold mb-2">Features:</h4>
                    <ul className="text-gray-400 group-hover:text-white transition-colors duration-300 text-sm list-disc pl-5 space-y-1">
                      {plan.features.map((feature, index) => (
                        <li key={index}>{feature}</li>
                      ))}
                    </ul>
                  </div>
                </div>
                <div className="mt-6">
                  <button className="w-full bg-gradient-to-r from-primary to-secondary text-white font-bold py-3 px-6 rounded-full hover:scale-105 transition-all duration-200 border-2 border-transparent hover:border-white">
                    Subscribe Now
                  </button>
                </div>
              </div>
            ))}
          </div>
        )}
      </div>
      <Footer />
    </div>
  );
};

export default SoftwareSubscriptionSection;
